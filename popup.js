...
function doQuestions(){
  var script = "document.getElementById('bodytext').value = 'prout'; fukya();"
  //var script = "var txtArea = document.getElementById('bodytext'); var txtNew = ''; var arr = txtArea.value.split('\n'); for (i=0;i<arr.length;i++){  if(arr[i] == ''){   txtNew += arr[i] + '\n';  }else{   if(!arr[i].match(/\./g)){    txtNew += '<b>' + arr[i] + '</b>\n';   }else{    txtNew += arr[i] + '\n';  }  } } txtArea.value = txtNew;";
  //chrome.tabs.executeScript(null, {file: 'addBold.js'});
  chrome.tabs.executeScript({
    file: 'addBold.js'
  });
}
